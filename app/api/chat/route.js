...
export async function POST(req) {
  ...
  let resultString = `\n**Top 3 Professor Recommendations for Best Communication:**\n`;
  results.matches.forEach((match, index) => {
    resultString += `
**${index + 1}. Professor ${match.metadata.name} (${match.metadata.subject}) - ${match.metadata.stars}/5**

- **Known for:** ${match.metadata.review}
- **Additional Info:** ${match.metadata.additionalInfo || 'No additional information available.'}
`;
  });
